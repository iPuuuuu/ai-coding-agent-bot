...
本服务可通过飞书机器人控制同一套 Claude 托管会话。飞书通道采用官方长连接接收事件，因此不需要公网 IP、反向代理或 Webhook URL。
...
- bot 托管的 Claude 会话回复会实时推送到飞书。
- 如果 Claude 进入“等待你输入 / 等待你选择”的状态，飞书会主动收到提醒。
- Claude 给出 2–4 个候选项时，机器人会发送交互卡片；点击按钮即可继续原会话。
