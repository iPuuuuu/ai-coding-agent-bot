...
- 第二轮完善：任务忙时自动排队（`/queue` 查看/清空）、会话与最近窗口状态持久化（`logs/state.json`，重启恢复）、日志轮转（10MB×5）。
- 第三轮修复：等待选择时文字回复优先于排队（不再把回复误入队列）；选择卡片改为非阻塞（按钮/文字单一续跑路径）；队列与**正在执行的任务**持久化（重启自动恢复）；飞书 ws 线程退出自动重连而不是进程退出；**回复目标 chat_id 持久化**（重启后仍知道发给谁）；过滤 Codex 非 JSON 噪音行。
- 第四轮：**多会话并行控制**——从「全局单任务串行」改为「每会话独立并行」：
  - `MAX_PARALLEL_TASKS`（默认 3）限制同时运行的 codex 子进程数；每会话独立 asyncio 任务、独立排队（全局 FIFO 保证同会话内先后顺序）、独立「等待回复」状态（`pending_by_session`）。
  - `@会话前8位 消息` 前缀路由 + 焦点模型：普通消息发给焦点会话，@ 前缀直接指挥任意会话；`@<id>` 单独一条 = 切换焦点；新增 `/focus` 等价命令。
  - `/sessions` 改为每会话一张操作卡片（详情 / 焦点 / 停止 / 接管）；新增 `sess:*` 卡片回调。
  - `/stop` 支持指定会话（`/stop <前8位>`）与全部（`/stop all`，同时清空队列）；停止改为定向终止对应 codex 子进程。
  - 接管本机任意 Codex 会话（含终端手动开的）；正在别处运行的会话拒绝接管；旧 Claude 会话保持只读。
  - 并行输出按会话标注 `[会话 abc12345]`；`agent_runner` 改造为按 run_key 管理进程/停止/最新文本。
  - 状态持久化带 `session_key`（队列任务），重启恢复各会话队列。
- 主要取舍：保持 `workspace-write` 沙箱（项目内可写、项目外只读），高风险操作靠系统提示词先问用户；需要联网/装依赖的命令会被沙箱拦截，属于预期行为。
- 尚未实现：定时任务、语音转文字、按 chat_id 完全隔离的多群并行。
